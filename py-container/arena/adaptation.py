            ...
            try:
                ...
                if parent_class_name:
                    ...
                    if parent_class_name in instantiated_classes:
                        print(
                            f"Using already instantiated class {parent_class_name}.")
                        parent_class_instance = instantiated_classes[parent_class_name]
                    else:
                        print(
                            f"Trying to instantiate class {parent_class_name}.")
                        if adaptationHandler.classes[parent_class_name].__len__() == 0:
                            print(
                                f"No constructors found for class {parent_class_name}.")
                        else:
                            print(
                                f"Contructor(s) found for class {parent_class_name}.")
                ...
                print(
                    f"The function '{moduleFunctionName}' throws an error: {e}.")
                success = False
                break
            else:
                ...
                if "Name" in neededAdaptations:
                    adapt_function_name(
                        submodule, new_function, interfaceMethodName)

                if "Return" in neededAdaptations:
                    adapt_return_type(submodule, new_function, interfaceMethodName,
                                      adaptationHandler.interfaceMethods[interfaceMethodName].returnType)
...
def main(interfaceSpecification: InterfaceSpecification, path, module, stimulus_sheet):
    ...
    adaptationHandler = AdaptationHandler(
        interfaceSpecification, moduleUnderTest, excludeClasses=True)
